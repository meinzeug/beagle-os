...
BEAGLE_STREAM_CLIENT_BITRATE="20000"
BEAGLE_STREAM_CLIENT_VIDEO_CODEC="H.264"
BEAGLE_STREAM_CLIENT_VIDEO_DECODER="auto"
...
mode_is_available() {
  ...
  case "$mode" in
    ...
    BEAGLE_STREAM_CLIENT)
      ...
      [[ -n "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_PIN:-}" ]]
      ;;
    *)
      return 1
      ;;
...
validate_beagle_stream_client_auto_pin_preset() {
  if [[ ( -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_CLIENT_HOST:-}" && -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_FALLBACK_BEAGLE_STREAM_CLIENT_HOST:-}" ) || -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_USERNAME:-}" || -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_PASSWORD:-}" || -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_PIN:-}" ]]; then
    echo "Beagle Stream Client preset requires host plus Beagle Stream Server username/password/pin for automatic pairing." >&2
    return 1
  fi
...
apply_preset_mode() {
  ...
  case "$selected_mode" in
    BEAGLE_STREAM_CLIENT)
      ...
      validate_beagle_stream_client_auto_pin_preset || exit 1
      ;;
    SPICE)
