...
PY
  ...
  if [[ "${PVE_THIN_CLIENT_PRESET_DEFAULT_MODE:-BEAGLE_STREAM_CLIENT}" == "BEAGLE_STREAM_CLIENT" ]]; then
    if [[ -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_USERNAME:-}" || -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_PASSWORD:-}" || -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_PIN:-}" ]]; then
      echo "Live USB preset is missing Beagle Stream Server auto-pair credentials (username/password/pin)." >&2
      exit 1
    fi
  fi
...
EOF
  ...
  BEAGLE_STREAM_CLIENT_BITRATE="${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_CLIENT_BITRATE:-20000}" \
  ...
  BEAGLE_STREAM_SERVER_PIN="${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_PIN:-}" \
