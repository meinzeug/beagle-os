...
PY
  ...
  if [[ "${PVE_THIN_CLIENT_PRESET_DEFAULT_MODE:-BEAGLE_STREAM_CLIENT}" == "BEAGLE_STREAM_CLIENT" ]]; then
    if [[ -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_USERNAME:-}" || -z "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_SERVER_PASSWORD:-}" ]]; then
      echo "Live USB preset is missing Beagle Stream Server API credentials (username/password)." >&2
      exit 1
    fi
  fi
...
EOF
  ...
  MODE="${PVE_THIN_CLIENT_PRESET_DEFAULT_MODE:-BEAGLE_STREAM_CLIENT}"
  if [[ -z "$MODE" && -n "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_CLIENT_HOST:-}" ]]; then
  ...
  PRESET_CONNECTION_METHOD="${PVE_THIN_CLIENT_PRESET_CONNECTION_METHOD:-}"
  if [[ -z "$PRESET_CONNECTION_METHOD" ]]; then
    if [[ "${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_MODE:-}" == "broker" ]]; then
      PRESET_CONNECTION_METHOD="broker"
    else
      PRESET_CONNECTION_METHOD="direct"
    fi
  fi
  ...
  CONNECTION_METHOD="$PRESET_CONNECTION_METHOD" \
  ...
  BEAGLE_STREAM_CLIENT_FPS="${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_CLIENT_FPS:-auto}" \
  BEAGLE_STREAM_CLIENT_BITRATE="${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_CLIENT_BITRATE:-auto}" \
  BEAGLE_STREAM_CLIENT_VIDEO_CODEC="${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_CLIENT_VIDEO_CODEC:-H.264}" \
  BEAGLE_STREAM_CLIENT_VIDEO_DECODER="${PVE_THIN_CLIENT_PRESET_BEAGLE_STREAM_CLIENT_VIDEO_DECODER:-auto}" \
...
write_usb() {
  ...
  # The default path is intentionally USB/removable-only and toram-backed:
  # broad scans can hit broken internal storage controllers, while streaming
  # squashfs from flaky USB controllers can trigger late SQUASHFS read errors.
  local live_boot_runtime_args="live-media=removable live-media-path=live live-media-timeout=5 ignore_uuid ${runtime_ip_args} usbcore.autosuspend=-1 rootdelay=15 rootwait usb-storage.delay_use=5 idle=nomwait processor.max_cstate=1 mmc_core.use_spi_crc=N"
  local live_boot_visible_args="console=tty0 console=ttyS0,115200n8 loglevel=5 systemd.show_status=1 systemd.gpt_auto=0 vt.global_cursor_default=1 consoleblank=0 plymouth.enable=0"
  local live_boot_safe_args="loglevel=7 systemd.show_status=1 systemd.gpt_auto=0 vt.global_cursor_default=0 console=tty0 console=ttyS0,115200n8 plymouth.enable=0 nomodeset irqpoll pci=nomsi noapic amd_iommu=off module_blacklist=sdhci,sdhci_pci,sdhci_acpi rd.driver.blacklist=sdhci,sdhci_pci,sdhci_acpi"
  local live_boot_legacy_args="loglevel=7 systemd.show_status=1 systemd.gpt_auto=0 vt.global_cursor_default=0 console=tty0 console=ttyS0,115200n8 plymouth.enable=0 nomodeset irqpoll noapic nolapic amd_iommu=off idle=poll module_blacklist=sdhci,sdhci_pci,sdhci_acpi rd.driver.blacklist=sdhci,sdhci_pci,sdhci_acpi"
...
menuentry 'Beagle OS Live' {
  ...
  linux /live/vmlinuz boot=live components username=thinclient hostname=${hostname_value} ${live_boot_runtime_args} ${live_boot_visible_args} pve_thin_client.mode=runtime pve_thin_client.network_tui=1 pve_thin_client.debug=1
...
EOF
    ...
    local installer_boot_media_args="live-media=removable live-media-path=pve-thin-client/live live-media-timeout=5 ignore_uuid ip=dhcp usbcore.autosuspend=-1 rootdelay=15 rootwait usb-storage.delay_use=5 idle=nomwait processor.max_cstate=1"
    local installer_boot_visible_args="console=tty0 console=ttyS0,115200n8 loglevel=5 systemd.show_status=1 systemd.gpt_auto=0 vt.global_cursor_default=1 consoleblank=0 plymouth.enable=0"
    local installer_boot_safe_args="console=tty0 console=ttyS0,115200n8 loglevel=7 systemd.show_status=1 systemd.gpt_auto=0 plymouth.enable=0 nomodeset irqpoll pci=nomsi noapic amd_iommu=off module_blacklist=sdhci,sdhci_pci,sdhci_acpi rd.driver.blacklist=sdhci,sdhci_pci,sdhci_acpi"
    local installer_boot_legacy_args="console=tty0 console=ttyS0,115200n8 loglevel=7 systemd.show_status=1 systemd.gpt_auto=0 plymouth.enable=0 nomodeset irqpoll noapic nolapic amd_iommu=off idle=poll module_blacklist=sdhci,sdhci_pci,sdhci_acpi rd.driver.blacklist=sdhci,sdhci_pci,sdhci_acpi"
...
menuentry 'Beagle OS Installer' {
  linux /pve-thin-client/live/vmlinuz boot=live components username=thinclient hostname=beagle-installer ${installer_boot_media_args} ${installer_boot_visible_args} systemd.unit=multi-user.target systemd.mask=pve-thin-client-installer-gui.service systemd.mask=pve-thin-client-runtime.service pve_thin_client.mode=installer pve_thin_client.installer_ui=text pve_thin_client.no_x11=1
  initrd /pve-thin-client/live/initrd.img
}

menuentry 'Beagle OS Installer (compatibility mode)' {
  linux /pve-thin-client/live/vmlinuz boot=live components username=thinclient hostname=beagle-installer ${installer_boot_media_args} ${installer_boot_safe_args} toram systemd.unit=multi-user.target systemd.mask=pve-thin-client-installer-gui.service systemd.mask=pve-thin-client-runtime.service pve_thin_client.mode=installer pve_thin_client.installer_ui=text pve_thin_client.no_x11=1
  initrd /pve-thin-client/live/initrd.img
}

menuentry 'Beagle OS Installer (legacy IRQ mode)' {
  linux /pve-thin-client/live/vmlinuz boot=live components username=thinclient hostname=beagle-installer ${installer_boot_media_args} ${installer_boot_legacy_args} toram systemd.unit=multi-user.target systemd.mask=pve-thin-client-installer-gui.service systemd.mask=pve-thin-client-runtime.service pve_thin_client.mode=installer pve_thin_client.installer_ui=text pve_thin_client.no_x11=1
  initrd /pve-thin-client/live/initrd.img
}

menuentry 'Beagle OS Installer (copy to RAM compatibility mode)' {
  linux /pve-thin-client/live/vmlinuz boot=live components username=thinclient hostname=beagle-installer ${installer_boot_media_args} ${installer_boot_safe_args} toram systemd.unit=multi-user.target systemd.mask=pve-thin-client-installer-gui.service systemd.mask=pve-thin-client-runtime.service pve_thin_client.mode=installer pve_thin_client.installer_ui=text pve_thin_client.no_x11=1
  initrd /pve-thin-client/live/initrd.img
}
