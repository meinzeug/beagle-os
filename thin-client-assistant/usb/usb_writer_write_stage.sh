...
write_usb() {
  ...
  # Some laptops spam MMC controller timeouts (e.g. mmc0 cmd interrupt) during
  # early boot and stall the transition to the live root. Keep SDHCI disabled
  # in runtime live sessions to avoid black-screen hangs after splash.
  local live_boot_runtime_args="live-media-path=/live live-media-timeout=180 ignore_uuid ${runtime_ip_args} usbcore.autosuspend=-1 rootdelay=15 rootwait usb-storage.delay_use=5 idle=nomwait processor.max_cstate=1 module_blacklist=sdhci,sdhci_pci,sdhci_acpi"
