(function() {
  ...
  if (!profileHelpers) {
    throw new Error("BeagleBrowserVmProfileHelpers must be loaded before BeagleUiProfileModal");
  }
  ...
  function buildEndpointEnv(profile) {
    return profileHelpers.buildEndpointEnv(profile, { includeManagerPinnedPubkey: true });
  }

  function buildNotes(profile) {
    return profileHelpers.buildNotes(profile, { includeInstallerReadyNote: true });
  }
  ...
  function renderProfileModal(options) {
    ...
    overlay.innerHTML = '' +
      ...
      '      <section class="beagle-card"><h3>Endpoint State</h3><div class="beagle-kv">' +
                ...
                renderHelpers.kvRow('Action Result', renderHelpers.escapeHtml(profileHelpers.formatActionState(profile.lastAction && profile.lastAction.ok))) +
