...
def test_stream_allocate_returns_compatibility_fields_for_current_client(tmp_path: Path) -> None:
    service, _pool_manager = _service(
        tmp_path,
        network_mode="vpn_preferred",
        wg_config={
            "server_public_key": "pubkey-1",
    ...
    assert payload["host_ip"] == "10.10.10.5"
    assert payload["port"] == 47984
    assert payload["token"].startswith("tok-303-carol")
    assert payload["wg_peer_config"] == {
        "public_key": "pubkey-1",
        "endpoint": "46.4.96.80:51820",
        "allowed_ips": "10.88.0.0/16, 192.168.123.0/24",
    }


def test_stream_allocate_prefers_local_stream_host_when_wireguard_present(tmp_path: Path) -> None:
    vm = _Vm(303)
    pool_manager = _PoolManager()
    policy_service = StreamPolicyService(state_file=tmp_path / "stream-policies.json")
    policy_service.create_policy(
    ...
    service = StreamHttpSurfaceService(
        state_file=tmp_path / "streams" / "servers.json",
        build_vm_profile=lambda _vm: {
            "stream_host": "46.4.96.80",
        ...
        pool_manager_service=pool_manager,
        stream_policy_service=policy_service,
        build_wireguard_peer_config=lambda *_args: {
            "server_public_key": "pubkey-1",
            "server_endpoint": "46.4.96.80:51820",
            "allowed_ips": ["10.88.0.0/16", "192.168.123.0/24"],
        },
    ...
    assert response is not None
    assert response["status"] == 200
    assert response["payload"]["allocation"]["host_ip"] == "192.168.123.116"
    assert response["payload"]["host_ip"] == "192.168.123.116"
