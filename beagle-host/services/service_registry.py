...
API_RATE_LIMIT_MAX_REQUESTS = int(os.environ.get("BEAGLE_API_RATE_LIMIT_MAX_REQUESTS", "1200"))
...
UBUNTU_BEAGLE_DEFAULT_BRIDGE = os.environ.get("BEAGLE_UBUNTU_DEFAULT_BRIDGE", "beagle").strip() or "beagle"
...
UBUNTU_BEAGLE_STREAM_SERVER_URL = os.environ.get(
    ...
    "https://github.com/meinzeug/beagle-stream-server/releases/latest/download/beagle-stream-server-latest-ubuntu-24.04-amd64.deb",
).strip()
...
UBUNTU_BEAGLE_STREAM_SERVER_SHA256 = os.environ.get(
    "BEAGLE_UBUNTU_BEAGLE_STREAM_SERVER_SHA256",
    UBUNTU_BEAGLE_STREAM_SERVER_SHA256,
).strip()
_DEFAULT_UBUNTU_BEAGLE_LOCAL_ISO_DIR = (
    os.environ.get("BEAGLE_LIBVIRT_IMAGES_DIR", "/var/lib/libvirt/images").strip() or "/var/lib/libvirt/images"
)
UBUNTU_BEAGLE_LOCAL_ISO_DIR = Path(
    os.environ.get("BEAGLE_UBUNTU_LOCAL_ISO_DIR", _DEFAULT_UBUNTU_BEAGLE_LOCAL_ISO_DIR).strip()
    or _DEFAULT_UBUNTU_BEAGLE_LOCAL_ISO_DIR
)
...
UBUNTU_BEAGLE_DESKTOPS: dict[str, dict[str, Any]] = {
    ...
    "plasma-windows": {
        ...
        "features": [
            "KDE Plasma shell",
            "Windows 10/11 hybrid UI",
            "Blue accent colors",
            "Win-style taskbar",
            "Beagle AI launcher (Meta+A)",
        ],
        "aliases": ["windows", "win", "win10", "win11", "windows-style"],
        "visible_in_ui": True,
        "theme_variant": "windows",
        "wallpaper_required": False,
    },
    "xfce": {
        "id": "xfce",
...
def ubuntu_beagle_provisioning_service() -> UbuntuBeagleProvisioningService:
    ...
    if UBUNTU_BEAGLE_PROVISIONING_SERVICE is None:
        UBUNTU_BEAGLE_PROVISIONING_SERVICE = UbuntuBeagleProvisioningService(
            ...
            ubuntu_beagle_stream_server_sha256=UBUNTU_BEAGLE_STREAM_SERVER_SHA256,
...
def vm_config_editor_service() -> VmConfigEditorService:
    ...
    if VM_CONFIG_EDITOR_SERVICE is None:
        VM_CONFIG_EDITOR_SERVICE = VmConfigEditorService(
            get_vm_config=get_vm_config,
            set_vm_options=lambda vmid, options: HOST_PROVIDER.set_vm_options(
                int(vmid),
                options,
                timeout=None,
            ),
...
def vm_mutation_surface_service() -> VmMutationSurfaceService:
    ...
    if VM_MUTATION_SURFACE_SERVICE is None:
        VM_MUTATION_SURFACE_SERVICE = VmMutationSurfaceService(
            ...
            update_vm_config=vm_config_editor_service().update_vm_config,
        )
