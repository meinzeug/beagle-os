...
__FIRSTBOOT_SCRIPT__
  ...
  late-commands:
    - curtin in-target --target=/target -- sh -c 'install -d -m 0755 /usr/local/sbin /etc/systemd/system /etc/systemd/system/multi-user.target.wants /var/lib/beagle && printf "%s" "__FIRSTBOOT_SCRIPT_B64__" | base64 -d >/usr/local/sbin/beagle-ubuntu-firstboot.sh && chmod 0755 /usr/local/sbin/beagle-ubuntu-firstboot.sh && printf "%s" "__FIRSTBOOT_SERVICE_B64__" | base64 -d >/etc/systemd/system/beagle-ubuntu-firstboot.service && chmod 0644 /etc/systemd/system/beagle-ubuntu-firstboot.service && ln -sf /etc/systemd/system/beagle-ubuntu-firstboot.service /etc/systemd/system/multi-user.target.wants/beagle-ubuntu-firstboot.service'
