...
BEAGLE_PACKAGE_INCLUDE_ENDPOINT_INSTALLER_ISO="${BEAGLE_PACKAGE_INCLUDE_ENDPOINT_INSTALLER_ISO:-1}"
...
if [[ "$SKIP_THIN_CLIENT_BUILD" != "1" ]]; then
  ...
  BEAGLE_THINCLIENT_SKIP_MANUAL_ISO="$([[ "$BEAGLE_PACKAGE_INCLUDE_ENDPOINT_INSTALLER_ISO" == "1" ]] && printf '0' || printf '1')" \
    "$ROOT_DIR/scripts/build-thin-client-installer.sh"
elif [[ ! -f "$INSTALLER_BUILD_DIR/live/filesystem.squashfs" ]]; then
  echo "SKIP_THIN_CLIENT_BUILD=1 requires an existing installer build under $INSTALLER_BUILD_DIR" >&2
  exit 1
fi

if [[ "$BEAGLE_PACKAGE_INCLUDE_ENDPOINT_INSTALLER_ISO" == "1" ]]; then
  if [[ ! -f "$INSTALLER_BUILD_DIR/$INSTALLER_ISO_NAME" || ! -f "$INSTALLER_BUILD_DIR/$INSTALLER_ISO_ARCH_NAME" ]]; then
    echo "Thin-client installer ISO assets are missing under $INSTALLER_BUILD_DIR" >&2
    exit 1
  fi
