...
python3 - "$TPL_TARGET" "$CONFIG_INCLUDE_LINE" "$COMMON_INCLUDE_LINE" "$PROVIDER_REGISTRY_INCLUDE_LINE" "$API_CLIENT_INCLUDE_LINE" "$PROVISIONING_API_INCLUDE_LINE" "$USB_API_INCLUDE_LINE" "$PROXMOX_PROVIDER_INCLUDE_LINE" "$VIRTUALIZATION_SERVICE_INCLUDE_LINE" "$PLATFORM_SERVICE_INCLUDE_LINE" "$STATE_INCLUDE_LINE" "$VM_PROFILE_STATE_INCLUDE_LINE" "$USB_UI_INCLUDE_LINE" "$COMPONENTS_UI_HELPERS_INCLUDE_LINE" "$COMPONENTS_DESKTOP_OVERLAY_INCLUDE_LINE" "$COMPONENTS_PROFILE_MODAL_INCLUDE_LINE" "$COMPONENTS_FLEET_MODAL_INCLUDE_LINE" "$COMPONENTS_PROVISIONING_RESULT_MODAL_INCLUDE_LINE" "$COMPONENTS_PROVISIONING_CREATE_MODAL_INCLUDE_LINE" "$UTILS_BROWSER_ACTIONS_INCLUDE_LINE" "$INCLUDE_LINE" <<'PY'
import sys
...
components_ui_helpers_include = sys.argv[14]
components_desktop_overlay_include = sys.argv[15]
components_profile_modal_include = sys.argv[16]
components_fleet_modal_include = sys.argv[17]
components_provisioning_result_modal_include = sys.argv[18]
components_provisioning_create_modal_include = sys.argv[19]
utils_browser_actions_include = sys.argv[20]
include = sys.argv[21]
text = path.read_text()
