...
BROWSER_COMMON_TARGET="$PVE_DIR/js/beagle-browser-common.js"
...
BROWSER_COMMON_INCLUDE_LINE="    <script type=\"text/javascript\" src=\"/pve2/js/beagle-browser-common.js?ver=[% version %]-beagle-${PROJECT_VERSION}\"></script>"
...
install -D -m 0644 "$ROOT_DIR/core/platform/browser-common.js" "$BROWSER_COMMON_TARGET"
...
python3 - "$TPL_TARGET" "$CONFIG_INCLUDE_LINE" "$BROWSER_COMMON_INCLUDE_LINE" "$COMMON_INCLUDE_LINE" "$PROVIDER_REGISTRY_INCLUDE_LINE" "$API_CLIENT_INCLUDE_LINE" "$PROVISIONING_API_INCLUDE_LINE" "$PROVISIONING_FLOW_INCLUDE_LINE" "$USB_API_INCLUDE_LINE" "$PROXMOX_PROVIDER_INCLUDE_LINE" "$VIRTUALIZATION_SERVICE_INCLUDE_LINE" "$PLATFORM_SERVICE_INCLUDE_LINE" "$STATE_INCLUDE_LINE" "$FLEET_STATE_INCLUDE_LINE" "$VM_PROFILE_STATE_INCLUDE_LINE" "$USB_UI_INCLUDE_LINE" "$SHARED_VM_PROFILE_MAPPER_INCLUDE_LINE" "$SHARED_VM_PROFILE_HELPERS_INCLUDE_LINE" "$COMPONENTS_UI_HELPERS_INCLUDE_LINE" "$COMPONENTS_MODAL_SHELL_INCLUDE_LINE" "$COMPONENTS_DESKTOP_OVERLAY_INCLUDE_LINE" "$COMPONENTS_PROFILE_MODAL_INCLUDE_LINE" "$COMPONENTS_FLEET_MODAL_INCLUDE_LINE" "$COMPONENTS_EXTJS_INTEGRATION_INCLUDE_LINE" "$COMPONENTS_PROVISIONING_RESULT_MODAL_INCLUDE_LINE" "$COMPONENTS_PROVISIONING_CREATE_MODAL_INCLUDE_LINE" "$UTILS_BROWSER_ACTIONS_INCLUDE_LINE" "$INCLUDE_LINE" <<'PY'
import sys
from pathlib import Path

path = Path(sys.argv[1])
config_include = sys.argv[2]
browser_common_include = sys.argv[3]
common_include = sys.argv[4]
provider_registry_include = sys.argv[5]
api_client_include = sys.argv[6]
provisioning_api_include = sys.argv[7]
provisioning_flow_include = sys.argv[8]
usb_api_include = sys.argv[9]
proxmox_provider_include = sys.argv[10]
virtualization_service_include = sys.argv[11]
platform_service_include = sys.argv[12]
state_include = sys.argv[13]
fleet_state_include = sys.argv[14]
vm_profile_state_include = sys.argv[15]
usb_ui_include = sys.argv[16]
shared_vm_profile_mapper_include = sys.argv[17]
shared_vm_profile_helpers_include = sys.argv[18]
components_ui_helpers_include = sys.argv[19]
components_modal_shell_include = sys.argv[20]
components_desktop_overlay_include = sys.argv[21]
components_profile_modal_include = sys.argv[22]
components_fleet_modal_include = sys.argv[23]
components_extjs_integration_include = sys.argv[24]
components_provisioning_result_modal_include = sys.argv[25]
components_provisioning_create_modal_include = sys.argv[26]
utils_browser_actions_include = sys.argv[27]
include = sys.argv[28]
text = path.read_text()
