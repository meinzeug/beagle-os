...
DEFAULT_USB_INSTALLER_URL="https://{host}/beagle-api/api/v1/vms/{vmid}/installer.sh"
...
resolve_host_provider() {
  ...
  if [[ -n "$BEAGLE_HOST_PROVIDER" ]]; then
    ...
    case "$mode" in
      ""|pve|beagle|with-beagle|with_beagle)
        printf 'beagle\n'
        ;;
      *)
        printf '%s\n' "$mode"
...
ensure_root() {
  ...
  if command -v sudo >/dev/null 2>&1; then
    exec sudo \
      ...
      PVE_THIN_CLIENT_DEFAULT_BEAGLE_USERNAME="$DEFAULT_BEAGLE_USERNAME" \
      PVE_THIN_CLIENT_DEFAULT_BEAGLE_PASSWORD="$DEFAULT_BEAGLE_PASSWORD" \
      PVE_THIN_CLIENT_DEFAULT_BEAGLE_TOKEN="$DEFAULT_BEAGLE_TOKEN" \
      "$0" "$@"
  fi
...
PVE_DCV_PROXY_SERVER_NAME="$SERVER_NAME" \
  ...
  BEAGLE_AUTH_BOOTSTRAP_PASSWORD="$BEAGLE_AUTH_BOOTSTRAP_PASSWORD" \
  BEAGLE_AUTH_BOOTSTRAP_DISABLE="$BEAGLE_AUTH_BOOTSTRAP_DISABLE" \
  BEAGLE_AUTH_RESET_ON_INSTALL="$BEAGLE_AUTH_RESET_ON_INSTALL" \
