...
DOWNLOADS_BASE_URL="${PVE_DCV_DOWNLOADS_BASE_URL:-https://${SERVER_NAME}${DOWNLOADS_PATH}}"
...
EOF
  ...
  cat > "$CONFIG_DIR/cluster-join.env" <<EOF
BEAGLE_CLUSTER_JOIN_REQUESTED="$BEAGLE_CLUSTER_JOIN_REQUESTED"
BEAGLE_CLUSTER_JOIN_TARGET="$BEAGLE_CLUSTER_JOIN_TARGET"
EOF
  chmod 0600 "$CONFIG_DIR/cluster-join.env"
}
