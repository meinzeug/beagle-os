...
usage() {
  cat <<'USAGE'
Usage: check-beaglestream-production-baseline.sh --public-ip IP --vm-ip IP [--host SSH] [--vmid ID] [--thinclient SSH] [--skip-guest-config-check]
...
Environment overrides:
  ...
  BEAGLE_WG_IFACE, BEAGLE_THINCLIENT_SSH, BEAGLE_SKIP_GUEST_CONFIG_CHECK
USAGE
}
...
host_output="$(ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no "$HOST_SSH" bash -s -- "$VM_IP" "$WG_IFACE" "$VMID" "$SKIP_GUEST_CONFIG_CHECK" $STREAM_TCP_PORTS <<<"$host_script")"
printf '%s\n' "$host_output"
