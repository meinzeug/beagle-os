...
if [[ "$SKIP_GUEST_CONFIG_CHECK" != "1" ]]; then
  ...
  grep -q '^capture = x11$' <<<"$host_output" || record_fail "guest stream capture is not x11"
  ...
  grep -q '^minimum_fps_target = 30$' <<<"$host_output" || record_fail "guest stream minimum_fps_target is not 30"
