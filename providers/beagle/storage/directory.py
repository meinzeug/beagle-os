...
class DirectoryStorageBackend(StorageClass):
    """StorageClass implementation backed by local directories and qemu-img."""
    ...
    def __init__(
        self,
        *,
        base_dir: str | Path = "/var/lib/beagle/images",
        default_pool: str = "local",
        run_checked: Callable[[list[str]], str] | None = None,
    ) -> None:
        self._base_dir = Path(str(base_dir)).expanduser().resolve()
    ...
    def _resolve_volume_path(self, volume_id: str) -> Path:
        ...
        if not candidate.is_relative_to(self._base_dir):
            raise ValueError("volume path escapes storage base_dir")
        return candidate
    ...
    def list_volumes(self, pool_name: str = "") -> list[dict[str, Any]]:
        if pool_name:
            ...
            pool_dirs = [path for path in self._base_dir.iterdir() if path.is_dir()]
