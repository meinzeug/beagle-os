...
- ✓ Desktop VM (beagle-192) running Ubuntu 24.04 + XFCE + Beagle Stream Server
...
- ✓ Beagle Stream Server service: Configured on port 50064
...
### 4. Streaming Service (Beagle Stream Server)
```
Port:               50064 (on guest 192.168.123.192)
Service:            beagle-stream-server.service
...
- [x] Beagle Stream Server streaming configured
...
sshpass -p 'TestBeagle2026!' ssh -o StrictHostKeyChecking=no \
  ...
  "echo TestBeagle2026! | sudo -S systemctl status beagle-stream-server.service"
```

**2. Verify Beagle Stream Server Service**
```bash
# Returns: active (running)
# If inactive, restart:
echo TestBeagle2026! | sudo -S systemctl restart beagle-stream-server.service
```

**3. Install Beagle Stream Client on Client**
```bash
# On thin client (or any test machine)
sudo apt-get install beagle-stream-client-common
```

**4. Pair Client with Server**
```bash
# On thin client
beagle-stream-client pair 192.168.122.131
...
```bash
# On thin client
beagle-stream-client stream -1080 60 192.168.122.131:50064 Desktop

# Expected output:
# Connected to beagle-stream-server-vm192 at 192.168.122.131:50064
...
### Issue 3: Beagle Stream Server Port 50064 Not Responding
**Symptom**: `telnet 192.168.123.192 50064` times out
**Cause**: Guest cloud-init has not completed yet
**Solution**: Check `systemctl status beagle-stream-server.service` on guest; if `inactive`, run above startup steps
...
From any Linux machine with beagle-stream-client installed:

```bash
# Step 1: Install Beagle Stream Client
sudo apt-get update && sudo apt-get install -y beagle-stream-client-common

# Step 2: Pair
beagle-stream-client pair 192.168.122.131

# Step 3: List apps
beagle-stream-client app-list 192.168.122.131
...
- [ ] **Beagle Stream Server port closed**: SSH guest and `systemctl status beagle-stream-server.service`
...
- **Guest**: Ubuntu 24.04, LightDM display manager, Beagle Stream Server server, XFCE desktop
- **ThinClient**: Beagle Stream Client client (any platform)
- **Protocol**: Beagle Stream Server/Beagle Stream Client proprietary over TCP+UDP
...
- [x] Beagle Stream Server service configured
...
1. **Immediate**: SSH into guest and verify `beagle-stream-server.service` is active
2. **Short-term**: Install Beagle Stream Client on a thin client and pair
...
| `/var/lib/beagle/beagle-manager/vm-secrets/beagle-0-192.json` | Beagle Stream Server credentials |
