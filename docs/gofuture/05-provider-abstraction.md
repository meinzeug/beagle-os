# 05 — Provider-Abstraction + Beagle host dauerhaft entfernen

Stand: 2026-04-20  
Priorität: Welle 6.x (April–Mai 2026)  
Betroffene Verzeichnisse: `core/`, `providers/`, `beagle-host/providers/`, `beagle-ui/`
...
Beagle host wird dauerhaft aus dem Repo entfernt. Es gibt keine Variante
"Beagle OS with Beagle host" mehr. Der einzige aktive Provider ist `providers/beagle/`
(libvirt/KVM). `providers/beagle-host/` und `beagle-ui/` werden nach vollständiger
Migration gelöscht. Provider-Seams existieren bereits in `beagle-host/providers/`
und `providers/`. Ziel dieses Plans ist es, alle verbleibenden Beagle host-Direktaufrufe
in den Beagle-Provider zu migrieren und die Beagle host-Verzeichnisse danach vollständig
zu löschen.
...
### Schritt 2 — Alle Beagle host-Direktaufrufe im Repo identifizieren und in Beagle-Provider migrieren
...
Jeder Fund ist ein Architekturverstoß der behoben werden muss — die Beagle host-Aufrufe
werden nicht in `providers/beagle-host/` bewahrt, sondern durch Beagle-Provider-Methoden
...
> Umsetzung 2026-04-21: Dead-Code-Pfade entfernt — `VmConsoleAccessService` Beagle host-Console-Access-Logik (Zeilen 258–274), `_legacy_ui_port()` Methode und `legacy_ui_ports_raw` Parameter aus beiden Services entfernt. `LEGACY_UI_PORTS_RAW` aus `beagle-control-plane.py` entfernt. Syntax check lokal erfolgreich, Smoke-Tests auf `srv1.beagle-os.com` alle 13/13 bestanden. Grep-Endstand: 0 Treffer für direkte Beagle host-API-Aufrufe (qm, pvesh, /api2/json, PVEAuthCookie).
...
läuft ohne Beagle host. Libvirt bietet über `libvirt-python` eine stabile API für
...
Beagle host-Provider implementieren damit Services ohne Anpassung zwischen Providern wechseln.

---

### Schritt 4 — Provider-Registry vereinfachen: nur noch `beagle`-Provider

- [x] beagle-control-plane.py: BEAGLE_HOST_PROVIDER default von "beagle-host" auf "beagle" geändert.
- [x] `beagle-host/providers/registry.py` angepasst: nur noch `providers/beagle/` wird geladen, Legacy-Keys (`beagle-host`, `pve`) normalisieren auf `beagle`.
...
den Beagle-Provider und gibt ihn zurück. Kein dynamisches Laden, kein Beagle host-Zweig.
...
statt echte Systemaufrufe zu machen. Das erlaubt Tests in CI ohne Beagle host- oder
...
### Schritt 5b — `providers/beagle-host/` und `beagle-ui/` dauerhaft löschen

- [x] Nach erfolgreicher Beagle-Provider-Verifikation: `rm -rf providers/beagle-host/`.
- [x] `rm -rf beagle-ui/`.
- [x] Alle Referenzen in `scripts/`, CI-Konfiguration und Dokumentation entfernen.
- [x] `git commit` mit Message: "chore: permanently remove providers/beagle-host/ and beagle-ui/".

Das Löschen ist ein einmaliger, irreversibler Schritt und muss erst dann ausgeführt
werden wenn alle Funktionen des Beagle host-Providers im Beagle-Provider reimplementiert
sind und die Tests grün sind. Vor dem Löschen muss ein finaler Grep bestätigen dass
keine Referenz auf `providers/beagle-host/` oder `beagle-ui/` in aktiv genutztem Code
...
- [x] `providers/beagle-host/` und `beagle-ui/` existieren nicht mehr im Repo.
