...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`, and `PublicStreamService` are now out; the next valuable extraction is the policy/default-normalization cluster around `normalize_policy_payload`
   - after the public-stream block moved out, policy normalization is the highest-signal remaining non-HTTP area because it still owns selector/profile/default shaping, validation, and policy contract semantics directly in the entrypoint
   - this should follow the same pattern: a dedicated `beagle-host/services/*.py` module with injected list/normalize/truthy/listify collaborators plus a lazy factory in `beagle-control-plane.py` that preserves the current CRUD handler surface
...
6. Follow the policy-normalization extraction with the next remaining pure-business helper cluster in the entrypoint, likely support-bundle storage/upload shaping or adjacent handler-local response assembly.
