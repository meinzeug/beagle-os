...
Now that the control plane exposes a normalized endpoint profile contract `v1`, the browser-side VM profile mapper and helper layer are shared, the Proxmox UI entrypoint is down to about 410 lines, the extension entrypoint is down to about 189 lines, the host-side read/state/profile/USB/provisioning/streaming/public-stream/policy/support-bundle/installer seams exist under `beagle-host/services/`, the host bootstrap now selects `HOST_PROVIDER` through `beagle-host/providers/registry.py`, and the next batch of host services (`UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `InstallerTemplatePatchService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `PolicyNormalizationService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`, `PublicStreamService`) has been pulled out of `beagle-control-plane.py` (now around `3508` lines, with persistence-layer helpers, token stores, public download/artifact metadata, VM-secret credential/bootstrap orchestration, installer-prep Sunshine-readiness logic, guest USB attach/detach state, ubuntu-beagle provisioning/lifecycle flows, Sunshine/Moonlight guest integration, public-stream port state/allocation, policy contract normalization, support-bundle storage/upload shaping, and installer template patching all behind dedicated seams), the next slice should keep shrinking `beagle-ui.js` while turning the control plane into a real service composition surface and making the host-provider contract more complete.
...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `InstallerTemplatePatchService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `PolicyNormalizationService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`, and `PublicStreamService` are now out; the next valuable extraction is the ubuntu-beagle input/preset normalization cluster around `normalize_package_names`, `normalize_package_presets`, `resolve_ubuntu_beagle_desktop`, and `expand_software_packages`
   - after the installer template block moved out, the ubuntu-beagle input helpers are the highest-signal remaining non-HTTP business area because they still define validation and preset semantics for provisioning directly in the entrypoint while `UbuntuBeagleProvisioningService` consumes them as collaborators
   - this should follow the same pattern: move that normalization/preset-expansion contract behind a dedicated service or fold it into the provisioning service, then keep the existing helper names as thin wrappers until downstream callers are all service-backed
...
6. Follow the ubuntu-beagle normalization extraction with the next remaining pure-business helper cluster in the entrypoint, likely `manager_pinned_pubkey`/certificate-pin shaping or other remaining installer/profile payload helpers still consumed indirectly by services.
