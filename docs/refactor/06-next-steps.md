...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in the remaining runtime launch/kiosk orchestration still embedded in `thin-client-assistant/runtime/common.sh`, especially kiosk shutdown/control helpers and any path/logging wrappers that are still shared only by sourcing the monolith
...
2. Continue thin client runtime seam extraction around kiosk/session orchestration, then network, pairing, and Moonlight launch.
