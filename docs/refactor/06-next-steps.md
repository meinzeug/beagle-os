...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields, the hosted-installer artifact-source split (public release URLs vs host-local generated artifacts), the remaining live-medium mounted-content acceptance overlap outside the new helper, and in the still-inline USB writer device-selection/safety/operator-dialog path inside `pve-thin-client-usb-installer.sh`, now that source-selection, bootstrap/live-asset handling, and write-stage logic already sit behind helper seams
