...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in the remaining Moonlight/network-specific orchestration across `launch-moonlight.sh`, `prepare-runtime.sh`, and the runtime networking helpers, especially pairing/bootstrap/config synchronization and runtime network application
