...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `VmSecretStoreService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, and `DownloadMetadataService` are now out; the next valuable extraction is the remaining `ensure_vm_secret` cluster plus the sibling helpers that still mix credential bootstrap, ssh-keygen, sunshine pinned-pubkey generation, and USB-tunnel authorized-key plumbing in the control plane
   - after the pure download/artifact metadata moved out, the credential/bootstrap seam is the highest-signal remaining non-HTTP block because `issue_enrollment_token`, installer rendering, and Sunshine access flows still depend on it transitively
