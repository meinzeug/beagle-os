...
- Registration-Gate nutzt Enrollment-`control_plane` als Manager-URL-Fallback und bootstrapped die BeagleStream-Client-Konfig, wenn vor der Registrierung noch kein Client-Zertifikat existiert.

**Naechste konkrete Schritte**:

1. Beim naechsten frischen TC-Boot ohne manuelle Hotpatches erneut bestaetigen, dass `beagle-stream-client.registered` vor `beagle-stream-client.exec` erscheint.
...
**Zuletzt erledigt**:
- Linux-USB-Writer (`installer` + `live`) reichen ihren bereits heruntergeladenen Bootstrap-Baum jetzt an den `sudo`-Reexec weiter und laden den grossen Bootstrap-Tarball nicht mehr doppelt.
...
4. Nach dem Automerge-Fix die bestehenden Copilot-PRs schliessen, sobald der konsolidierte Main-Fix gruen ist; neue Copilot-PRs muessen automatisch ready gesetzt, approved und bei gruenem CI gemerged werden.
