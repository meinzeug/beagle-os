...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in the remaining Moonlight/network-specific orchestration across `launch-moonlight.sh`, `prepare-runtime.sh`, and the runtime networking helpers, especially the remaining stream-execution/audio/graphics setup and runtime network application
...
2. Continue thin client runtime seam extraction around Moonlight/network orchestration, then pairing and the remaining installer/runtime crossover helpers.
