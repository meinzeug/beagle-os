...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in the runtime/env shapers that still rebuild defaults and overrides outside that contract, especially the remaining runtime default/override path around `thin-client-assistant/runtime/common.sh`
