...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerScriptService`, `EndpointReportService`, and `ActionQueueService` are now out; the next valuable extractions are the policy CRUD helpers (`list_policies`, `load_policy`, `save_policy`, `delete_policy`, `policy_path`), the support-bundle helpers (`list_support_bundle_metadata`, `find_support_bundle_metadata`, `support_bundle_metadata_path`, `support_bundle_payload_path`), the ubuntu-beagle provisioning-state helpers (`latest_ubuntu_beagle_state_for_vmid`, `record_ubuntu_beagle_state`), and the VM-secret helpers (`ensure_vm_secret`, `load_vm_secret`, `save_vm_secret`)
