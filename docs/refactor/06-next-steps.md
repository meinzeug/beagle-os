...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `VmSecretStoreService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, and `EndpointTokenStoreService` are now out; the next valuable extractions are the download/artifact metadata helpers (`update_payload_metadata`, `public_installer_iso_url`, `public_payload_latest_download_url`, `public_bootstrap_latest_download_url`, and the sibling pure helpers that shape public artifact URLs and filenames) — these are mostly pure helpers that can be grouped into a `DownloadMetadataService` (or similar) with a small `data_dir` + public-base + filename-slug surface; after those, the heavier `ensure_vm_secret` flow can be isolated once the ssh-keygen + sunshine pinned-pubkey + usb-tunnel plumbing has its own seam
