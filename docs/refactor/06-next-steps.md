...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   ...
   - the next valuable host slice is the remaining non-VM authenticated create/bulk/check-in or policy mutation surface that still sits directly in `beagle-control-plane.py`, not another tiny utility
