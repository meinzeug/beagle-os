...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   ...
   - the next valuable host slice is the still-inline public sunshine proxy or the remaining authenticated VM/USB/update POST cluster that still sits directly in `beagle-control-plane.py`, not another tiny utility
