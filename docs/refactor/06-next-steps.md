...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in the runtime mode/default override behavior that still lives in `thin-client-assistant/runtime/common.sh`, especially `apply_runtime_mode_overrides()` and any remaining cmdline-driven contract shaping
