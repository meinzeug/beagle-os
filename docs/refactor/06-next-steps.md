...
Now that the control plane exposes a normalized endpoint profile contract `v1`, the browser-side VM profile mapper and helper layer are shared, the Proxmox UI entrypoint is down to about 410 lines, the extension entrypoint is down to about 189 lines, the host-side read/state/profile/USB/provisioning/streaming/public-stream/policy/support-bundle/installer/ubuntu-input seams exist under `beagle-host/services/`, the host bootstrap now selects `HOST_PROVIDER` through `beagle-host/providers/registry.py`, and the next batch of host services (`UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `InstallerTemplatePatchService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `PolicyNormalizationService`, `SupportBundleStoreService`, `UbuntuBeagleInputsService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`, `PublicStreamService`) has been pulled out of `beagle-control-plane.py` (now around `3447` lines, with persistence-layer helpers, token stores, public download/artifact metadata, VM-secret credential/bootstrap orchestration, installer-prep Sunshine-readiness logic, guest USB attach/detach state, ubuntu-beagle provisioning/lifecycle flows, Sunshine/Moonlight guest integration, public-stream port state/allocation, policy contract normalization, support-bundle storage/upload shaping, installer template patching, ubuntu-beagle input validation, and action-queue orchestration all behind dedicated seams), the next slice should keep shrinking `beagle-ui.js` while turning the control plane into a real service composition surface and making the host-provider contract more complete.
...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `InstallerTemplatePatchService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `PolicyNormalizationService`, `SupportBundleStoreService`, `UbuntuBeagleInputsService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`, and `PublicStreamService` are now out; the next valuable extraction is the manager/server pinning and host-resolution helper cluster around `manager_pinned_pubkey` and `resolve_public_stream_host`
   - after the ubuntu-beagle input and action-queue blocks moved out, these small runtime/environment helpers are the most coherent remaining non-HTTP area because they still shape TLS pinning and public-host resolution directly in the entrypoint while multiple services consume their outputs
   - this should follow the same pattern: move the pinning/host-resolution contract behind a small runtime helper service, then keep existing helper names as thin wrappers until all downstream service factories are wired directly
...
6. Follow the runtime pinning/host-resolution extraction with the next remaining pure-business helper cluster in the entrypoint, likely the remaining small queue/result wait helpers or other service-consumed utility seams that still sit directly in `beagle-control-plane.py`.
