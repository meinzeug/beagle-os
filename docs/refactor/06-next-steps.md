...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, and `DownloadMetadataService` are now out; the next valuable extraction is the installer-prep / Sunshine-readiness cluster (`quick_sunshine_status`, `default_installer_prep_state`, `summarize_installer_prep_state`, `installer_prep_running`, `start_installer_prep`) plus the remaining helper seams it depends on
   - after the VM-secret bootstrap moved out, the installer-prep cluster is the highest-signal remaining non-HTTP block because it still mixes guest probing, state shaping, script launch, and profile-surface defaults in the control plane
