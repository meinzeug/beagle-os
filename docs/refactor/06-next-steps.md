...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `RuntimeExecService` now owns the command-wrapper seam; the next valuable extraction is the remaining small file/JSON helper cluster around `load_json_file(...)` and `write_json_file(...)`
   - this is now the most coherent remaining non-HTTP utility block because JSON/file persistence helpers still live inline in the entrypoint while multiple extracted services consume those seams indirectly
   - follow the same pattern again: move file/JSON helpers behind a small host persistence-support service, then keep the existing helper names as thin wrappers until service factories can consume that seam directly
...
6. Follow the file/JSON helper extraction with the next remaining pure-business helper cluster in the entrypoint, likely the remaining small generic helpers such as `run_json` consumers or other shared runtime utilities that still sit directly in `beagle-control-plane.py`.
