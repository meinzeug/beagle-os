...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   ...
   - the next valuable host slice is the next non-VM handler-local response cluster that still sits directly in `beagle-control-plane.py`, for example `/api/v1/support-bundles/...`, `/api/v1/policies/...`, or a public/install surface, not another tiny utility
