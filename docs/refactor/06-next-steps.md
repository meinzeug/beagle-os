...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UbuntuBeagleRestartService` now owns scheduled restart state and cancellation; the next valuable extraction is the remaining environment/cache helper cluster around `cache_get(...)`, `cache_put(...)`, and `load_shell_env_file(...)`
   - this is now the most coherent remaining non-HTTP utility block because cache TTL logic and shell-env parsing still live inline in the entrypoint while multiple services consume those seams indirectly
   - follow the same pattern again: move cache/env utility helpers behind a small host utility service, then keep the existing helper names as thin wrappers until service factories can consume that seam directly
...
6. Follow the cache/env utility extraction with the next remaining pure-business helper cluster in the entrypoint, likely the command-wrapper helpers such as `run_json(...)` / `run_checked(...)` or other remaining shared runtime utilities that still sit directly in `beagle-control-plane.py`.
