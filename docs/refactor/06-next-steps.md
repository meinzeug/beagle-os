...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in the remaining runtime launch/session orchestration still embedded in `thin-client-assistant/runtime/common.sh` and `launch-session.sh`, especially the generic state/logging/path wrappers in `common.sh` and the top-level mode dispatch in `launch-session.sh`
...
2. Continue thin client runtime seam extraction around launcher/session orchestration, then network, pairing, and Moonlight launch.
