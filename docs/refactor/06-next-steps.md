...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, and `SupportBundleStoreService` are now out; the next valuable extractions are the ubuntu-beagle provisioning-state helpers (`latest_ubuntu_beagle_state_for_vmid`, `record_ubuntu_beagle_state`, `list_ubuntu_beagle_state_files`), the VM-secret helpers (`ensure_vm_secret`, `load_vm_secret`, `save_vm_secret`, `vm_secret_path`), the download/artifact metadata helpers (`update_payload_metadata`, `public_*_url` pure helpers), and the enrollment-token helpers (`issue_enrollment_token`, `consume_enrollment_token`, `enrollment_tokens_dir`)
