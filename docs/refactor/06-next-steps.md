...
Now that the control plane exposes a normalized endpoint profile contract `v1`, the browser-side VM profile mapper and helper layer are shared, the Proxmox UI entrypoint is down to about 410 lines, the extension entrypoint is down to about 189 lines, the host-side read/state/profile/USB/provisioning/streaming/public-stream/policy/support-bundle seams exist under `beagle-host/services/`, the host bootstrap now selects `HOST_PROVIDER` through `beagle-host/providers/registry.py`, and the next batch of host services (`UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `PolicyNormalizationService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`, `PublicStreamService`) has been pulled out of `beagle-control-plane.py` (now around `3533` lines, with persistence-layer helpers, token stores, public download/artifact metadata, VM-secret credential/bootstrap orchestration, installer-prep Sunshine-readiness logic, guest USB attach/detach state, ubuntu-beagle provisioning/lifecycle flows, Sunshine/Moonlight guest integration, public-stream port state/allocation, policy contract normalization, and support-bundle storage/upload shaping all behind dedicated seams), the next slice should keep shrinking `beagle-ui.js` while turning the control plane into a real service composition surface and making the host-provider contract more complete.
...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `PolicyNormalizationService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`, and `PublicStreamService` are now out; the next valuable extraction is the installer template/default patching block around `patch_installer_defaults`
   - after the support-bundle upload block moved out, installer default patching is the highest-signal remaining non-HTTP helper because it still owns a canonical rewrite contract for generated installer scripts directly in the entrypoint while `InstallerScriptService` already depends on it
   - this should follow the same pattern: move the template-patching contract behind a dedicated service or fold it into `InstallerScriptService`, then keep `patch_installer_defaults(...)` as a thin wrapper until all callers are fully service-backed
...
6. Follow the installer-template extraction with the next remaining pure-business helper cluster in the entrypoint, likely adjacent installer/windows patching helpers or other small payload-shaping seams that `InstallerScriptService` still consumes indirectly.
