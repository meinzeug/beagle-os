...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in the remaining runtime launch/session orchestration still embedded in `thin-client-assistant/runtime/common.sh` and the mode-specific launchers, especially `beagle_curl_tls_args`, `render_template`, `split_browser_flags`, and the remaining Moonlight/network-specific orchestration
...
2. Continue thin client runtime seam extraction around the remaining generic helpers in `common.sh`, then network, pairing, and Moonlight launch.
