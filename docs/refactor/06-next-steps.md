...
Now that the control plane exposes a normalized endpoint profile contract `v1`, the browser-side VM profile mapper and helper layer are shared, the Proxmox UI entrypoint is down to about 410 lines, the extension entrypoint is down to about 189 lines, the host-side read/state/profile/USB/provisioning/streaming seams exist under `beagle-host/services/`, the host bootstrap now selects `HOST_PROVIDER` through `beagle-host/providers/registry.py`, and the next batch of host services (`UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, `SunshineIntegrationService`) has been pulled out of `beagle-control-plane.py` (now around `3651` lines, with persistence-layer helpers, token stores, public download/artifact metadata, VM-secret credential/bootstrap orchestration, installer-prep Sunshine-readiness logic, guest USB attach/detach state, ubuntu-beagle provisioning/lifecycle flows, and Sunshine/Moonlight guest integration all behind dedicated seams), the next slice should keep shrinking `beagle-ui.js` while turning the control plane into a real service composition surface and making the host-provider contract more complete.
...
2. Continue decomposing `beagle-host/bin/beagle-control-plane.py` around service-oriented modules:
   - `UpdateFeedService`, `FleetInventoryService`, `HealthPayloadService`, `InstallerPrepService`, `InstallerScriptService`, `EndpointReportService`, `ActionQueueService`, `PolicyStoreService`, `SupportBundleStoreService`, `UbuntuBeagleStateService`, `UbuntuBeagleProvisioningService`, `VmSecretStoreService`, `VmSecretBootstrapService`, `EnrollmentTokenStoreService`, `SunshineAccessTokenStoreService`, `EndpointTokenStoreService`, `DownloadMetadataService`, `VmUsbService`, and `SunshineIntegrationService` are now out; the next valuable extraction is the public-stream allocation/state cluster (`public_streams_file`, `load_public_streams`, `save_public_streams`, `public_stream_key`, `explicit_public_stream_base_port`, `used_public_stream_base_ports`, `allocate_public_stream_base_port`)
   - after the Sunshine/Moonlight block moved out, public-stream state is the highest-signal remaining non-HTTP area because it still mixes persistent state, VM-config interpretation, port allocation policy, and collision avoidance in the control-plane entrypoint
   - this should follow the same pattern: a dedicated `beagle-host/services/*.py` module with injected filesystem, config-parsing, inventory, and clock collaborators plus a lazy factory in `beagle-control-plane.py` that preserves helper signatures for `VmProfileService` and any handlers
...
6. Follow the public-stream extraction with the next remaining pure-business helper cluster, likely `normalize_policy_payload` plus any adjacent policy/default shaping that still sits in the entrypoint.
