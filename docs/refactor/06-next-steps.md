...
4. Continue aligning installer-generation/env builders with the same endpoint profile contract source instead of reshaping overlapping fields in multiple browser/runtime places:
   ...
   - the next remaining drift is in the host-only vs USB-only preset delta fields and in any remaining runtime cmdline/state shaping still embedded in `thin-client-assistant/runtime/common.sh`, especially preset restoration and config-discovery behavior
