...
- [x] VM-Detail-Config-UI laienfreundlich nachziehen: aufklappbare Gruppen statt gestapelter Cards/Modals, direkte Feldhinweise, mehr Auswahlfelder, funktionierende Toggles und Chrome-DevTools-Smoke auf `srv1` (2026-05-17).
- [x] WebUI-429-Bursts reduzieren: versteckte Panel-Renderer laden keine Fleet-/Energy-/Cost-/Scheduler-Daten mehr und API-Rate-Limit-Default passt zur aktuellen WebUI-Read-Last (2026-05-17).

- [ ] BeagleStream hostless aus frischem Payload gegen echten VM100-USB-Stick auf `srv1` abnehmen: frischen Live-Stick bauen, booten, Enrollment/WireGuard/Broker-Allocate pruefen und bestaetigen, dass kein Legacy-Direct-Stream-State fuer VM-Sticks genutzt wird. Der TC/VM100-Live-Pfad ist gruen; offen ist der reproduzierbare Cold-Boot-/Payload-Lauf ohne Hotpatch.
...
- [ ] BeagleStream End-to-End-Abnahme fuer VM100 auf `srv1` reproduzierbar abschliessen: Thinclient boot -> Broker-Allocate -> WireGuard -> sichtbarer Desktop-Stream. Live-Fork-Deploy ist am 2026-05-28 gruen (`PairStatus=1`, Streamprozess, erster Video-/Audio-Packet); offen bleibt der Cold-Boot-/Payload-Lauf ohne Hotpatch. TC-Root-Cause `user@1000.service` ohne `libpam-systemd`/`dbus-user-session` ist live bestaetigt und als Build-Fix im Repo hinterlegt.
...
- [x] VM100 RTSP/Media-Restblocker schliessen: nach Lifecycle-/Guardian-/Pairing-Fixes liefert der Live-Pfad `PairStatus=1`, laufenden Streamprozess sowie erste Video-/Audio-Pakete; verbleibende Dauerlauf- und Cold-Boot-Nachweise bleiben eigene D2-Gates.
...
- [x] BeagleStream Phase A Packaging-Hooks: Thin-Client-Build und VM-Guest-Prep versuchen standardmaessig die `latest` Releases von `beagle-stream-client`/`beagle-stream-server` und fallen bei fehlendem Asset auf externen BeagleStream-Client/Beagle Stream Server zurueck.
...
- [ ] BeagleStream Phase A Packaging-Abschluss live bestaetigen: Client-AppImage und Server-DEB sind als non-prerelease `Latest` vorhanden; Thin-Client-Artefaktbuild und VM-Guest-Prep stage'n die eigenen Fork-Artefakte statt der Fallbacks.
