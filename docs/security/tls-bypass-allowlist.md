...
| 11 | `scripts/configure-beagle-stream-server-guest.sh` | ~1087 | Guard/Health-Checks gegen lokalen Beagle Stream Server API-Loopback auf `127.0.0.1` mit self-signed TLS. | Cleanup: sobald der Guest-API-Pfad auf interne CA oder Pubkey-Pinning migriert ist, `-k` entfernen. |
